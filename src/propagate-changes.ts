import { ApplyData, getApplyNodeBlock } from "./apply-node.js";
...
: Change | undefined {
  for (let nodeIndex = startNodeIndex; nodeIndex < block.nodeOrder.length; ++nodeIndex) {
    ...
    if (node.type === 'argument') {
      ...
      const applyBlock = getApplyNodeBlock(node);
      ...
      change = propagateChanges(applyBlock, applyData.block, argumentChanges, encloserChanges);
    }
